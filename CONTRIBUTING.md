Contributing to Myntcore
=======

We're working hard to make *myntcore* the most powerful JavaScript library for working with Mynt. Our goal is to have *myntcore* be a library that can be used by anyone interested in Mynt, and to level expertise differences with great design and documentation.
...
These are some global design goals in myntcore that any change must adhere.
...
When possible, myntcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of myntcore methods. For example, calling `JSON.stringify` on an myntcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'myntcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the myntcore object.

DO:
```javascript
var myntcore = require('../');
var PublicKey = myntcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add joenilan git@github.com:joenilan/myntcore.git
git pull --rebase joenilan master
```
...
```
Finally go to [github.com/joenilan/myntcore](https://github.com/joenilan/myntcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of myntcore.

If you have any questions feel free to post them to
[github.com/joenilan/myntcore/issues](https://github.com/joenilan/myntcore/issues).
